...
describes what users get. Pick a format with `-o`, or let rdap choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
rdap <command> -o table   # aligned columns for reading
rdap <command> -o jsonl   # one JSON object per line, for piping
rdap <command> -o json    # a single JSON array
rdap <command> -o csv     # spreadsheet friendly
rdap <command> -o tsv     # tab-separated
rdap <command> -o url     # just the URL column
rdap <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
rdap <command> --fields id,title,url
```
...
```bash
rdap <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
rdap <command>            # a table, because this is a terminal
rdap <command> | wc -l    # JSONL, because this is a pipe
```
