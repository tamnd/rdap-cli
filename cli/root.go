// Package cli assembles the rdap command tree from the rdap
...
// NewApp assembles the kit application from the rdap domain. The
...
//
// To add a command, declare it in rdap/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := rdap.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(rdap.Domain{}).Register(app)
